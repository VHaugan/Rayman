...
public class Ferr2D_Path : MonoBehaviour
{
    ...
    {
        if (aSmoothed) return GetVertsSmoothed(aSplitDistance, aSplitCorners);
    ...
    public List<Vector2> GetVertsSmoothed(float aSplitDistance, bool aSplitCorners)
    {
        ...
        if (aSplitCorners)
        {
            ...
            if (closed) CloseEnds(ref segments, aSplitCorners);
            if (segments.Count > 1)
            {

...
public static class Ferr2DT_Triangulator
{
	...
	public  static List<int> GetIndices           (ref List<Vector2> aPoints, bool aTreatAsPath, bool aInvert) {
        ...
        tris.Add(aPoints.Count - 1);
        tris.Add(aPoints.Count - 2);
        tris.Add(aPoints.Count - 3);
        tris.Add(aPoints.Count - 1);
        tris.Add(aPoints.Count - 3);
        tris.Add(aPoints.Count - 4);
		
		// add in all the vers of the path
        for (int i = 0; i < aPoints.Count - 4; i += 1)
        {
            int tri = GetSurroundingTri(aPoints, tris, aPoints[i]);
            ...
            if (tri != -1)
            {
				...
				EdgeFlip(aPoints, tris, tri);
				EdgeFlip(aPoints, tris, tris.Count-3);
				EdgeFlip(aPoints, tris, tris.Count-6);
            }
		}
		
		// hacky solution to the stack overflow on the recursive edge flipping I was getting
		for (int i = 0; i < tris.Count*2; i+=3) {
			EdgeFlip(aPoints,tris, i%tris.Count);
		}
		
		// remove the encompassing triangles
        if (!aInvert) aPoints.RemoveRange(aPoints.Count - 4, 4);
		List<int> result     = new List<int>();
        int       invertMesh = aInvert ? 0 : 1;
		for (int i=0;i<tris.Count;i+=3) {
			if (aInvert || 
               (tris[i  ] < aPoints.Count &&
				tris[i+1] < aPoints.Count &&
				tris[i+2] < aPoints.Count)) {

				Vector2 center = (aPoints[tris[i]] + aPoints[tris[i+1]] + aPoints[tris[i+2]]) / 3;
				if (!aTreatAsPath || (GetSegmentsUnder(aPoints, center.x, center.y, aInvert).Count/2) % 2 == invertMesh) {
					if (IsClockwise(aPoints[tris[i]], aPoints[tris[i+1]], aPoints[tris[i+2]])) {
						result.Add(tris[i+2]);
						result.Add(tris[i+1]);
						result.Add(tris[i  ]);
					} else {
